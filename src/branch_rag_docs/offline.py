"""Pipeline offline của nhánh PDF. Một file điều phối toàn bộ các chặng.
...
Đối xứng với `src/branch_sql/offline.py` — cùng cách nối chặng, cùng cách in
kết quả. Khác ở chỗ nhánh này chưa có chặng `link` và `verify`.
"""
...
def run(name: str, *, do_index: bool = True, do_images: bool = False) -> dict:
    ...
    from ..offline.chunk.text_chunker import ChunkConfig, process_chunking
    from ..offline.parse.pdf_parse import convert_pdf
    ...
    if do_images:
        from ..offline.extract.merge_documents import merge_documents
        from ..offline.parse.image_captioner import main as caption_main  # noqa: F401
        from ..offline.parse.image_extractor import main as extract_main  # noqa: F401
    ...
    # ---- chặng 5: embed + index
    if do_index:
        from ..offline.index.chroma_store import build_index

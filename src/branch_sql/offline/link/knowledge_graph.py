...
from ...config import KNOWLEDGE_DIR, PROCESSED_DIR, ROOT
...
def build(markdown_path, *, out_dir=None, model="llama3") -> dict:
    """Bước `knowledge` của chain trong `offline/pipeline.py`.

    markdown -> node-link JSON + HTML. Trả về đường dẫn và kích thước graph để
    `pipeline.main()` in ra, giống các bước khác.
    """
...
if __name__ == "__main__":
    # Chạy riêng trên toàn bộ markdown đã có, không qua offline/pipeline.py.
